...
bool initGLFW() {
	...
	window = glfwCreateWindow(800, 800, "Riggi_Christian_100752293", nullptr, nullptr);
...
struct Material
{
	...
	//New float variable is created in struct to control the new unifrom value
	float mixTexture;
};
...
int main() {
	...
	Texture2DData::sptr boxMap = Texture2DData::LoadFromFile("images/box.bmp");
	
	
	// Create a texture from the dataTexture2D::sptr diffuse = Texture2D::Create();diffuse->LoadData(diffuseMap);Texture2D::sptr specular = Texture2D::Create();specular->LoadData(specularMap);// Creating an empty textureTexture2DDescription desc = Texture2DDescription();desc.Width = 1;desc.Height = 1;desc.Format = InternalFormat::RGB8;Texture2D::sptr texture2 = Texture2D::Create(desc);texture2->Clear();
	...
	Texture2D::sptr boxTexture = Texture2D::Create();
	boxTexture->LoadData(boxMap);
	...
	while (!glfwWindowShouldClose(window)) {
		...
		for(int ix = 0; ix < 4; ix++) {
			...
			shader->SetUniform("u_mixTextures", materials[ix].mixTexture);
