...
{
	glViewport(0, 0, width, height);    windowSize = glm::ivec2(width, height); 
}
...
int main() {
	...
	vao->AddVertexBuffer(color_vbo, 
		{ 
		BufferAttribute( 1, 3, AttributeType::Float, 0, NULL ) 
		});
	...
	vao2->AddVertexBuffer(interleaved_vbo, { 
		BufferAttribute(0, 3, AttributeType::Float, stride, 0), BufferAttribute(1, 3, AttributeType::Float, stride, sizeof(float) * 3), 
		});   
